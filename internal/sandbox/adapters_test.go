...
func TestSelectBackendChoosesPlatformAdapterWithFallback(t *testing.T) {
		...
		backend := SelectBackend(BackendOptions{
			...
			LookupExecutable: func(name string) (string, error) {
				if name == WindowsSandboxCommandRunnerName {
					return `C:\pvyai\pvyai-windows-command-runner.exe`, nil
				}
				if name == WindowsSandboxSetupName {
					return `C:\pvyai\pvyai-windows-sandbox-setup.exe`, nil
				}
				return "", errors.New("missing")
			},
		})
		if backend.Name != BackendWindowsRestrictedToken || !backend.Available || backend.Executable != `C:\pvyai\pvyai-windows-command-runner.exe` {
	...
	t.Run("windows missing helper exes self-dispatch", func(t *testing.T) {
		...
		// backend self-dispatches via the running pvyai binary, so it is AVAILABLE
		// rather than failing every command. Pin os.Executable for determinism.
		restore := osExecutable
		osExecutable = func() (string, error) { return `C:\pvyai\pvyai.exe`, nil }
		...
		if backend.Executable != `C:\pvyai\pvyai.exe` {
