...
func TestSandboxManagerBuildsCommandPlanThroughWindowsRunner(t *testing.T) {
	...
	backend := Backend{Name: BackendWindowsRestrictedToken, Available: true, Executable: `C:\pvyai\pvyai-windows-command-runner.exe`, Platform: "windows"}
	policy := DefaultPolicy()
	...
	if !plan.Wrapped || plan.Name != `C:\pvyai\pvyai-windows-command-runner.exe` || plan.TargetBackend != BackendWindowsRestrictedToken {
...
func TestSandboxManagerSelectsPlatformBackend(t *testing.T) {
	tests := []struct {
		...
		{name: "windows", goos: "windows", lookupName: WindowsSandboxCommandRunnerName, lookupPath: `C:\pvyai\pvyai-windows-command-runner.exe`, setupPath: `C:\pvyai\pvyai-windows-sandbox-setup.exe`, want: BackendWindowsRestrictedToken, wantTarget: BackendWindowsRestrictedToken},
