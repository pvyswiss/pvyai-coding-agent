...
func (m model) mcpManagerOverlay(width int) string {
	...
	state := m.mcpViewState()
	lines := []string{
		fillPaletteLine(pvyaiTheme.ink.Bold(true).Render(pluralCount(len(state.Servers), "server")), innerWidth, transparentSurface),
		fillPaletteLine(renderMCPManagerSearchLine(m.mcpManager.query, innerWidth), innerWidth, transparentSurface),
		pvyaiTheme.accent.Bold(true).Render("User MCPs"),
	}
	if len(state.Servers) == 0 {
		lines = append(lines, pvyaiTheme.faint.Render("  No MCP servers configured."))
	}
	...
	lines = append(lines, pvyaiTheme.line.Render(strings.Repeat("─", innerWidth)))
	lines = append(lines, fillPaletteLine(pvyaiTheme.faint.Render("type search   up/down navigate   Enter action   Alt+d disable   Esc close"), innerWidth, transparentSurface))
	return centerRenderedBlock(styledBlockFillTitle(overlayWidth, "Manage MCP servers", lines, pvyaiTheme.lineStrong, lipgloss.NewStyle()), width)
}
...
func renderMCPManagerSearchLine(query string, width int) string {
	...
	if query == "" {
		return fitStyledLine(prompt+pvyaiTheme.faint.Render("MCP servers, tools, docs..."), width)
	}
	return fitStyledLine(prompt+pvyaiTheme.ink.Render(query), width)
}

func (m model) renderMCPManagerItemLines(width int, items []mcpManagerItem) ([]string, []int) {
	if len(items) == 0 {
		return []string{fillPaletteLine(pvyaiTheme.faint.Render("  no MCP actions"), width, transparentSurface)}, []int{-1}
	}
	...
	for offset, item := range visible {
		...
		if group := mcpManagerItemGroup(item.Kind); group != "" && group != lastGroup {
			lines = append(lines, pvyaiTheme.accent.Bold(true).Render(group))
		...
		left := marker + surface(pvyaiTheme.ink).Render(item.Label)
		right := ""
		if item.Meta != "" {
			right = surface(pvyaiTheme.faint).Render(item.Meta)
		}
...
func (m model) mcpManagerSelectionDetail(width int) []string {
	...
	case mcpManagerItemServer:
		...
		lines := []string{
			fillPaletteLine(pvyaiTheme.ink.Bold(true).Render(server.Name)+" "+pvyaiTheme.faint.Render(server.Transport+" · "+server.State), width, transparentSurface),
		}
		if target := strings.TrimSpace(server.Target); target != "" {
			lines = append(lines, fitStyledLine(pvyaiTheme.faint.Render(target), width))
		}
		action := "Alt+d disable"
		if strings.EqualFold(strings.TrimSpace(server.State), "disabled") {
			action = "Alt+e enable"
		}
		lines = append(lines, fillPaletteLine(pvyaiTheme.faint.Render("Enter check   Alt+c check   "+action+"   Alt+r remove"), width, transparentSurface))
		return lines
	case mcpManagerItemMarketplace:
		lines := []string{
			fillPaletteLine(pvyaiTheme.ink.Bold(true).Render(item.Label)+" "+pvyaiTheme.faint.Render(item.Meta), width, transparentSurface),
		}
		if detail := firstMCPMarketplaceDetailLine(item); detail != "" {
			lines = append(lines, fitStyledLine(pvyaiTheme.faint.Render(detail), width))
		}
		lines = append(lines, fillPaletteLine(pvyaiTheme.faint.Render("Enter fills composer: "+item.InstallCommand), width, transparentSurface))
		return lines
	case mcpManagerItemAddRemote:
		return []string{fillPaletteLine(pvyaiTheme.faint.Render("Enter fills composer: /mcp add <name> --url <url>"), width, transparentSurface)}
	case mcpManagerItemAddStdio:
		return []string{fillPaletteLine(pvyaiTheme.faint.Render("Enter fills composer: /mcp add <name> -- <command> [args...]"), width, transparentSurface)}
	case mcpManagerItemList:
		return []string{fillPaletteLine(pvyaiTheme.faint.Render("Enter runs pvyai mcp list and refreshes this manager."), width, transparentSurface)}
	default:
		return nil
	}
}
