...
func TestRunPluginAddListRemove(t *testing.T) {
	...
	if !strings.Contains(stdout.String(), "pvyai.demo") {
	...
	stdout.Reset()
	stderr.Reset()
	if exit := runWithDeps([]string{"plugin", "remove", "pvyai.demo"}, &stdout, &stderr, deps); exit != 0 {
		t.Fatalf("plugin remove exit = %d, stderr = %s", exit, stderr.String())
	}
	if _, err := os.Stat(filepath.Join(pluginsDir, "pvyai.demo")); err == nil {
