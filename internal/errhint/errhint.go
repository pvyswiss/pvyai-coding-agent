...
// `pvyai exec` provider-error path (CLI) can append one concrete next step instead
...
// CLIHint returns a one-line hint referencing `pvyai …` subcommands, or "" when the
// category is Unknown. Meant for the non-interactive `pvyai exec` error path, where
// slash commands don't apply.
func CLIHint(err error) string {
	switch Classify(err) {
	case Auth:
		return "API key rejected — run `pvyai auth` or set the provider's API key, then retry"
	case RateLimit:
		return "Rate limited — wait a moment, or switch model with --model"
	case Connectivity:
		return "Can't reach the provider — run `pvyai doctor`"
	case ModelNotFound:
		return "Model unavailable — run `pvyai doctor` or pick another with --model"
	case ContextOverflow:
