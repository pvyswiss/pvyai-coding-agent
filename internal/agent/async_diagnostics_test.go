...
func TestRunDeliversAsyncDiagnosticsNudgeNextTurn(t *testing.T) {
	...
	provider := &mockProvider{
		turns: [][]pvyruntime.StreamEvent{
			{
				{Type: pvyruntime.StreamEventToolCallStart, ToolCallID: "call-1", ToolName: "fake_edit"},
				{Type: pvyruntime.StreamEventToolCallDelta, ToolCallID: "call-1", ArgumentsFragment: `{}`},
	...
	for _, message := range provider.requests[1].Messages {
		if message.Role == pvyruntime.MessageRoleTool && strings.Contains(message.Content, "boom") {
	...
	found := false
	for _, message := range provider.requests[1].Messages {
		if message.Role == pvyruntime.MessageRoleUser && strings.HasPrefix(message.Content, asyncDiagnosticsNudge) {
...
func TestRunFinalizationGateDeliversLateDiagnostics(t *testing.T) {
	...
	provider := &mockProvider{
		turns: [][]pvyruntime.StreamEvent{
			...
			{
				{Type: pvyruntime.StreamEventText, Content: "done after fix"},
				{Type: pvyruntime.StreamEventDone},
			},
		},
	}
