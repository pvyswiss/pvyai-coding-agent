...
func TestInspectSummarizesChangesAndRedactsDiff(t *testing.T) {
	...
	runner := &fakeRunner{results: []CommandResult{
		...
		{Stdout: "feature/m5\n"},
		{Stdout: "abc1234\n"},
		{Stdout: " M internal/verify/verify.go\x00?? internal/pvygit/pvygit.go\x00"},
		{Stdout: "abc1234\n"},
		{},
		{},
	...
	if summary.Files[1].Path != "internal/pvygit/pvygit.go" || summary.Files[1].Status != "untracked" || !summary.Files[1].Untracked {
...
func TestInspectPreviewIncludesUntrackedOnlyChanges(t *testing.T) {
	...
	writeTestFile(t, filepath.Join(root, "notes.md"), "hello pvyai\n")
	...
	if !strings.Contains(summary.Diff, "diff --git a/notes.md b/notes.md") || !strings.Contains(summary.Diff, "+hello pvyai") {
...
func TestCreatePRCommandConstruction(t *testing.T) {
	t.Run("CreatePRWithAllOptions", func(t *testing.T) {
		root := t.TempDir()
		runner := &fakeRunner{results: []CommandResult{
			{Stdout: "https://github.com/pvyswiss/pvyai-coding-agent/pull/123\n"},
		}}
		...
		if result.Output != "https://github.com/pvyswiss/pvyai-coding-agent/pull/123\n" {
	...
	t.Run("CreatePRMinimal", func(t *testing.T) {
		root := t.TempDir()
		runner := &fakeRunner{results: []CommandResult{
			{Stdout: "https://github.com/pvyswiss/pvyai-coding-agent/pull/124\n"},
		}}
