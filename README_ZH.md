...
PVYai 是一个用于本地终端的 AI 编码智能体。它可以检查仓库、编辑文件、运行命令、使用浏览器/终端辅助工具，并在你选择模型和权限级别的同时保持持久的本地会话。

```bash
pvyai
pvyai exec "修复 ./pkg 中失败的测试"
...
- **保持控制权。** 文件写入、Shell 命令、网络访问和工作区外写入都经过 PVYai 的权限和沙箱策略。
- **在终端中工作。** TUI 具有模型/提供商选择器、图片输入、斜杠命令、实时计划/工具渲染、回滚滚动、主题以及恢复/分叉支持。
- **无 TUI 也能工作。** `pvyai exec` 可脚本化，支持文本/JSON/stream-JSON I/O、隔离的工作树、规范优先运行，以及用于 CI 的有意义的退出码。
- **保持上下文本地化。** 会话存储在磁盘上，可搜索、可恢复，且 PVYai 不会作为遥测数据上传。
...
npm 包安装一个小型包装器以及与你平台匹配的 PVYai 二进制文件（从 GitHub Releases 获取）。支持 Linux、macOS 和 Windows 的 x64 和 arm64。

### Bun

Bun 默认不运行依赖的生命周期脚本，因此获取 PVYai 二进制文件的 `postinstall` 会被跳过，首次运行会失败并显示 `No native binary found next to the npm wrapper`。
...
其他方式：在 `bun add` 之前将 `"trustedDependencies": ["@pvyswiss/pvyai-agent"]` 添加到项目的 package.json 以提前允许 postinstall；或在不支持 `bun pm trust` 的旧版 Bun 上手动运行安装程序（`node node_modules/@pvyswiss/pvyai-agent/scripts/postinstall.mjs`）。
...
go build -o pvyai-seccomp ./cmd/pvyai-seccomp   # 可选的兼容性包装器
```

将 `pvyai` 和 `pvyai-linux-sandbox` 放在 `PATH` 上的同一目录中（`~/.local/bin` 是一个好的默认选择）。macOS 不需要额外的辅助二进制文件。Windows 源码构建可以使用主 `pvyai.exe` 作为沙箱辅助程序；发布包仍然附带独立的 Windows 辅助可执行文件。
...
对于本地模型，运行 Ollama 或 LM Studio，然后使用 `pvyai setup` 或 `pvyai providers detect`。
...
```bash
pvyai exec "解释 internal/agent/loop.go"
pvyai exec --model claude-sonnet-4.5 "重构配置加载器"
pvyai exec --use-spec "为 API 客户端添加速率限制"
pvyai exec --worktree "在隔离的工作树中尝试迁移"
pvyai exec --resume
pvyai exec --fork <session-id> "尝试另一种方法"
```
...
- 不安全/自主模式是显式选择加入。
- 在 PVYai 控制的界面上，密钥会从工具输出和日志中被脱敏。

示例：

```bash
pvyai --add-dir ../docs-site
pvyai exec --add-dir ../shared "更新两个仓库"
```
...
PVYai 包含本地文件/搜索/编辑/Shell 工具、用于公共 URL 的 `web_fetch`，以及用于额外工具的 MCP 支持。

对于本地开发服务器，使用 Shell 命令（如通过 `exec_command` 的 `curl`），这样正常的沙箱和权限策略就会生效。长时间运行的命令保持附加到后台终端会话，可以从 TUI 中列出或停止。

npm 包还包含本地浏览器/终端工具使用的浏览器和终端辅助包。源码构建可以在它们位于 `PATH` 上或在 PVYai 的本地控制设置中配置时使用相同的辅助工具。

## 常用命令

```text
pvyai                  交互式 TUI
pvyai exec             一次性或脚本化智能体运行
pvyai setup            首次运行提供商设置
pvyai auth             支持提供商的 OAuth/登录辅助
pvyai models           模型注册表和能力
pvyai providers        提供商配置和检测
pvyai doctor           设置、密钥和连接检查
pvyai context          上下文预算报告
pvyai repo-map         确定性仓库映射
pvyai repo-info        本地仓库摘要
pvyai search | find    搜索本地会话历史
pvyai sessions         检查、恢复、分叉和回滚会话
pvyai spec             管理规范模式草稿
pvyai specialist       管理专业子智能体
pvyai skills           管理 Markdown 指令技能
pvyai plugins          管理插件
pvyai hooks            管理生命周期钩子
pvyai mcp              管理 MCP 服务器和工具
pvyai serve --mcp      通过 MCP stdio 暴露 PVYai 工具
pvyai sandbox          检查沙箱策略和授权
pvyai worktrees        准备隔离的 Git 工作树
pvyai verify           检测和运行本地验证检查
pvyai changes          检查和提交本地 Git 变更
pvyai usage            Token 使用量和估算成本
pvyai cron             定时智能体任务
pvyai update           检查更新版本
```
